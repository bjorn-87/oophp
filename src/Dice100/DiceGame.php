...
class DiceGame
{
    ...
    * @var string $name Name of the player.
     ...
     */
    public function __construct(string $name = "Human", int $dices = 3, int $hands = 2, int $winScore = 100, int $sides = 6)
    {
        $this->hands = $hands;
        $this->players = [];
        $this->totalScore = [];
        ...
        for ($i=0; $i < $hands; $i++) {
            $this->players[] = new DiceHand($dices, $sides);
            $this->totalScore[$i] = 0;
            if ($i === 0) {
        ...
        $this->histogram->injectData($this->players[$this->currentPlayer]);
    }
    ...
    public function roll()
    {
        ...
        if ($this->currentPlayer === 0) {
            ...
            $this->computerRoll(
                $this->totalScore[$this->currentPlayer],
                $this->totalScore[0],
                $this->currentScore
            );
        }
     ...
     * @return void
     */
    public function computerRoll(int $scoreComputer = 0, int $scorePlayer = 0, int $roundScore = 0)
    {
        $this->rollCount = 0;
        $notOne = true;
        $round = new DiceRound($this->players[$this->currentPlayer]);
        ...
        while ($notOne) {
            ...
            if ($hasOne) {
                $notOne = false;
            } elseif ($scoreComputer + $roundScore >= $this->winScore) {
                $notOne = false;
            } elseif ($scorePlayer >= 80) {
                if ($roundScore > 25) {
                    $notOne = false;
                } else {
                    $notOne = true;
                }
            } elseif ($roundScore > 18) {
                $notOne = false;
            } elseif ($round->getScore() > 12) {
                $notOne = false;
            }
        }
        ...
        $this->gotOne = $hasOne;
        if (!$hasOne) {
            $this->totalScore[$this->currentPlayer] += $roundScore;
        }
    }
